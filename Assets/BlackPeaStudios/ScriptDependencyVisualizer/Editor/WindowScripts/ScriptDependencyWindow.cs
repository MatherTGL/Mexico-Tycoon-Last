...
namespace BPS
{
	public class ScriptDependencyWindow : EditorWindow
	{
		...
		static void LoadTextures(bool forced = false)
		{
			if (forced)
			{
				_regenerateButtonTexture = UnityEngine.Resources.Load("RegenerateButton") as Texture;
				_settingsButtonTexture = UnityEngine.Resources.Load("SettingsButton") as Texture;
				_loadLayoutButtonTexture = UnityEngine.Resources.Load("LoadLayoutButton") as Texture;
				_saveLayoutButtonTexture = UnityEngine.Resources.Load("SaveLayoutButton") as Texture;
				_centerButtonTexture = UnityEngine.Resources.Load("CenterButton") as Texture;
		...
		void DrawUIElements()
		{
			...
			GUI.DrawTexture(new Rect(10f, 10f, 100f, 100f), UnityEngine.Resources.Load("AssetLogo") as Texture);
			...
			else
			{
				if (GUI.Button(centerRect, _centerButtonTexture))// "Center"))
					ReCenter();
				if (GUI.Button(generateRect, _regenerateButtonTexture))
				{// "Generate"))
					weaveStartTime = Time.realtimeSinceStartup;
					InitScriptData(true);
				}
				if (GUI.Button(saveRect, _saveLayoutButtonTexture))
				{// "SaveLayout")) {
					if (m_forceData != null)
				...
				if (GUI.Button(restoreRect, _loadLayoutButtonTexture))
				{// "RestoreLayout")) {
					InitScriptData();
				}
			}
			if (m_forceData != null)
			{
				...
				if (!m_forceData.m_withinThreshold)
				{
					...
					if (GUI.Button(new Rect(position.size.x * .2f + 60f, position.size.y - 25f, 20f, 20f), UnityEngine.Resources.Load("StopIcon") as Texture))
						m_forceData._count = m_forceData._maxCycles;
					Repaint();
			...
			if (!settingsPanelActive)
			{
				...
				if (GUI.Button(new Rect(posX - 54.5f, 10.5f, 43f, 43f), UnityEngine.Resources.Load("SearchButton") as Texture))
				{//, "Find")) {
